...
def extract_from_image(client: anthropic.Anthropic, img_b64: str, job_id: str = "", pg: int = 0, retries: int = 3) -> dict:
    for attempt in range(retries):
        try:
            ...
            if result:
                return result
            # Log o que Claude retornou para debug
            if job_id:
                _log(job_id, f"   ⚠️ Pág {pg+1}: Claude não retornou JSON válido. Resposta: {text[:200]}")
            return _empty()
        ...
        for i, pdf_path in enumerate(pdf_files):
            ...
            for pg in range(n_pages):
                ...
                try:
                    ...
                    _log(job_id, f"   Página {pg+1}/{n_pages} — extraindo com Claude...")
...
<main>
  ...
  <div class="card" id="api-key-section">
    <div class="card-title"><span class="icon">🔑</span> Chave da API Anthropic</div>
    <div id="key-env-ok" style="display:none" class="key-ok">✅ ANTHROPIC_API_KEY configurada via variável de ambiente.</div>
    <div id="key-input-area">
      <label for="api-key-input">Cole sua chave API (sk-ant-...)</label>
      <input type="password" id="api-key-input" placeholder="sk-ant-api03-..." autocomplete="off">
      <p class="hint">A chave é usada apenas nesta sessão e nunca armazenada. Para não digitar sempre, defina a variável de ambiente <code>ANTHROPIC_API_KEY</code>.</p>
    </div>
  </div>
...
async function startProcessing() {
  ...
  if (!apiKey) {
    ...
    if (!keyStatus.configured) {
      alert('Informe a chave da API Anthropic antes de continuar.');
      return;
    }
  }
...
if __name__ == "__main__":
    ...
    print(f"  API Key:    {'✓ configurada via env var' if key_set else '⚠ não definida (informe no navegador)'}")
